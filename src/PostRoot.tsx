...
export const PostRoot = ({ issueNumber }: { issueNumber: number }) => {
  ...
  const data: PostRoot_PostQueryResponse | null | undefined = useLazyLoadQuery<
    PostRoot_PostQuery
  >(
    query, // $FlowFixMe: expects persisted variables
    {
      issueNumber,
      ...
      <>
        <Head>
          ...
          {config.siteHostname || config.vercelUrl ? (
            <meta
              key="og:image"
              property="og:image" // n.b. Ok to use vercel url for og-image as a fallback, but
