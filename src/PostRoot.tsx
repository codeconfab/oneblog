...
import type { PostRoot_PostQuery, PostRoot_PostQueryResponse } from "./__generated__/PostRoot_PostQuery.graphql";
...
export const PostRoot = ({
  ...
  const data: PostRoot_PostQueryResponse | null | undefined = useLazyLoadQuery<PostRoot_PostQuery>(query, // $FlowFixMe: expects persisted variables
  {
    issueNumber
  ...
  if (!gitHub || !post || !labels || !labels.find(l => l && l.name.toLowerCase() === 'publish')) {
    ...
    return <>
        <Head>
          ...
          {config.siteHostname || config.vercelUrl ? <meta key="og:image" property="og:image" // n.b. Ok to use vercel url for og-image as a fallback, but
        // careful not to use it as a canonical url
        content={`${// $FlowFixMe: checked above
        config.siteHostname || config.vercelUrl}${basePath}/api/og-image/${post.number}`} /> : null}
          <meta key="type" property="og:type" content="article" />
          <meta key="description" property="description" content={description} />
          <meta key="og:description" property="og:description" content={description} />
        </Head>
        <Header gitHub={gitHub} adminLinks={[{
        label: 'Edit post',
        href: editIssueUrl({
