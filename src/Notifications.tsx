...
function CorsViolation({
  corsViolation
}) {
  return corsViolation ? <Header justify="center" background="status-error">
      <Box pad="small" align="center" width="large">
        ...
        textAlign: 'center'
          ...
          <Anchor color="white" target="_blank" rel="noreferrer noopener" href={`https://www.onegraph.com/dashboard/app/${config.appId}`}>
            OneGraph Dashboard
          </Anchor>
          .
        </Text>
      </Box>
    </Header> : // Empty header to prevent re-render
  <Header />;
}
  ...
  render() {
    ...
    return <>
        <NotificationContext.Provider value={this._notificationContext}>
          ...
          <Layer position="top-right" modal={false} responsive={false} plain margin={{
          vertical: 'medium',
          horizontal: 'small'
        }}>
            {Object.keys(notifications).map(k => <Box key={k} style={{
            alignSelf: 'flex-end'
          }} fill={false} margin="xsmall" align="center" direction="row" gap="small" justify="between" round="medium" elevation="medium" color="white" background={notifications[k].level === 'success' ? 'status-ok' : 'status-error'} pad={{
            vertical: 'small',
            horizontal: 'medium'
          }}>
                ...
                <Button icon={<FormClose />} onClick={() => this._removeNotification(k)} plain />
              </Box>)}
          </Layer>
