...
export function fetchTokenInfo({
  ...
  const getUrl = `https://sourcecodeshots.com/api/token-info?code=${encodeURIComponent(code)}&theme=${theme}&language=${language ? language : ''}`;
  const resp = getUrl.length < 2083 ? fetch(getUrl, {
    method: 'GET',
  ...
  return resp.then(r => r.json()).then(tokenInfo => {
    registerTokenInfo({
      code,
      theme,
      language,
